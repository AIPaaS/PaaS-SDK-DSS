...
public class DSSFactory {
	...
	public static IDSSClient getClient(AuthDescriptor ad) throws Exception {
		...
		if (DSSClients.containsKey(instanceKey)) {
		...
		String DSSConf = CCSComponentFactory.getConfigClient(authResult.getConfigAddr(),
				authResult.getConfigUser(), authResult.getConfigPasswd()).get(DSS_CONFIG_COMMON_PATH);
		String DSSRedisConf = CCSComponentFactory.getConfigClient(authResult.getConfigAddr(),
				authResult.getConfigUser(), authResult.getConfigPasswd()).get(DSS_CONFIG_PATH + srvId);
		Gson gson = new Gson();
		...
		String password = CiperUtil.decrypt(PWD_KEY, (String) DSSConfMap.get(MONGO_PASSWORD));
		String redisHosts = (String) DSSConfMap.get(MONGO_REDIS_HOST);
		// String DSSRWConf =
		// confBase.getConfigFromWritable(DSS_CONFIG_PATH+"/"+srvId);
		// String DSSRWConf =
		// CCSComponentFactory.getConfigClient(authResult.getConfigAddr(),
		// authResult.getConfigUser(),authResult.getConfigPasswd()).get
		// 封装成配置对象
		Map DSSRedisConfMap = gson.fromJson(DSSRedisConf, Map.class);
		String userId = authResult.getUserId();
		System.out.println();
		DSSClient = new DSSClient(hosts, userId, username, password, redisHosts, DSSRedisConfMap);
