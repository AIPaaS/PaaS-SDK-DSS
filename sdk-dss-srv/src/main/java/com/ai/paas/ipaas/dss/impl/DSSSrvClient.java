...
import com.ai.paas.ipaas.dss.MongoInfo;
...
public class DSSSrvClient implements IDSSClient {
    ...
    private static final String MONGO_DB_NAME = "dbName";
    private static final String MONGO_DB_SIZE = "size";
    private static final String MONGO_FILE_LIMIT_SIZE = "limitSize";
    ...
    public DSSSrvClient(String hosts, String userId, String username, String password, String redisHosts,
            Map<String, String> redisConf) throws Exception {
        this.bucket = redisConf.get(MONGO_DB_NAME);
        this.fileLimitSize = Double.parseDouble(redisConf.get(MONGO_FILE_LIMIT_SIZE));
        this.dbSize = Double.parseDouble(redisConf.get(MONGO_DB_SIZE));

        MongoInfo mongoInfo = new MongoInfo(hosts, userId, username, password, bucket);
        Gson gson = new Gson();
        // 需要变成json格式
        dssClient = DSSBaseFactory.getClient(gson.toJson(mongoInfo));
    ...
    private void judgeSize(Object obj) {
        ...
        if (DSSSrvHelper.okSize(DSSSrvHelper.tobyte(fileLimitSize), DSSSrvHelper.getSize(obj)) < 0) {
            log.error("file too large");
            throw new DSSRuntimeException(new Exception("file too large"));
        }

        if (DSSSrvHelper.okSize(DSSSrvHelper.okSize(DSSSrvHelper.tobyte(dbSize), usedSize),
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < l.size(); i++) {
            bytes[i] = BigDecimal.valueOf((double) l.get(i)).byteValue();
        }
    }
    ...
    public long updateById(String id, String doc) {
        Assert.notNull(id, "id is null");
        Assert.notNull(doc, "doc is null");
        return dssClient.updateById(id, doc);
    }
    ...
    public long update(String query, String doc) {
        ...
        Assert.notNull(doc, "doc is null");
        return dssClient.update(query, doc);
    }
    ...
    public long upsert(String query, String doc) {
        Assert.notNull(query, "query is null");
        Assert.notNull(doc, "doc is null");
        return dssClient.upsert(query, doc);
    }
